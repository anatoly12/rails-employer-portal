class EmployerPortal::Sync::Company
  ...
  def create_partner
    ::EmployerPortal::Sync::Partner.create(
      name: company.name,
      type_of: "CONSUMER",
  ...
  def new_access_code
    @new_access_code ||= loop do
      ...
      break raw if ::EmployerPortal::Sync::AccessCode.where(access_code: raw).limit(1).empty?
    end
  end

  def create_access_code(partner)
    ::EmployerPortal::Sync::AccessCode.create(
      partner: partner,
